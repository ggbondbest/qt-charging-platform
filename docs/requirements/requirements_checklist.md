...
[核心充电闭环 PR #6](https://github.com/ggbondbest/qt-charging-platform/pull/6)。
...
- [x] 预约超时由 Server 权威处理；
- [x] 有效预约才能开始充电；
- [x] 预约、订单、电桩状态在一个事务中进入充电；
- [x] Server 返回实时功率、电量、时长、金额快照；
- [ ] Client 展示实时功率、电量、时长、金额；
- [ ] 断线后可恢复查询当前充电状态；
- [x] 停止充电生成待支付订单并释放电桩；
- [x] 金额按订单电价快照和整数规则计算；
- [x] 余额不足不完成订单、不扣成负数；
- [x] 支付扣款与订单完成同事务；
- [x] 重复停止/重复支付不会重复更新或扣款；
- [ ] 历史订单可查询并查看详情；
- [ ] 后台能实时/刷新看到订单和桩状态变化。
...
- [x] 所有动态 SQL 使用 prepared query + bind；
- [x] Repository 不向 Client 泄露原始数据库错误；
- [x] 预约事务失败回滚；
- [x] 开始/停止充电事务失败回滚；
- [ ] 余额扣除/充值事务失败回滚；
- [ ] DB 锁定、磁盘错误和损坏有完整错误处理；
- [x] `foreign_key_check` 无行；
- [x] `integrity_check` 为 `ok`；
- [ ] schema 版本升级有迁移测试。
...
- [ ] 工作线程退出时安全关闭 DB 和对象；
- [x] malformed JSON 不导致 Server 崩溃；
- [x] 超大/零长度 frame 导致连接安全关闭；
- [x] 未知动作返回 `UNKNOWN_REQUEST_TYPE`；
- [x] 未授权动作返回 `UNAUTHORIZED`；
- [x] 错误响应和日志不包含密码、hash、salt、SQL 或本地路径；
- [x] 多 Client 同时连接测试；
...
- [x] Client/Server 全部 API 经 Qt 6.2.4 编译验证；
- [ ] Ubuntu 安装说明包含 Qt Base、Sqlite driver、Charts、WebEngine 的准确依赖；
- [x] clean clone 后无需本机绝对路径；
- [x] 删除 build 后按 README 可从零配置；
- [ ] 首次运行可创建数据库；
