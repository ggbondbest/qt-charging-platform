...
- AI参谋在线由模型规划受控检索，再结合当前发布聚合、模型报告和本地BM25项目知识生成带引用的回答；
  在线问候也由模型生成。网页保留外发说明，在线模式主动发送授权本次问题、有限历史、范围说明和检索上下文，不再逐次勾选同意框。
  本地全量聚合按问题形成最多48项成组分析证据，加模型报告后最多64项；16段口径/方法知识最多检索4段，不外发原始业务行、用户/会话明细或完整数据表。
...
规划与生成各最多两次尝试，格式/字段/截断/引用纠错共享每阶段的一次额外机会；正常两次、总计最多四次调用，共享60秒预算。
安全JSON容错、有限纠错和主动重试不能保证供应商持续可用，也不取代范围与引用核验；日志不记录问答或密钥。
