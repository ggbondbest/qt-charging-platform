...
| AI参谋 | [advisor](advisor/README.md)，已有聚合与报告的证据说明 | 智能分析子页；默认离线，复用同源FastAPI，不新增训练任务 |
...
- AI参谋默认用当前发布聚合和本地验证报告回答，来源缺失时明确说明；不能把自然语言说明当作新的评估或诊断。
  可选在线调用只发送白名单聚合指标，不发送原始问题、自由备注或逐条业务明细，详见专属 `ML_ADVISOR_*` 配置说明。
...
AI参谋入口：`POST /api/v1/intelligence/advisor`，与网页同源；离线运行不需要Key，不能启动旧的独立参谋服务代替统一入口。
