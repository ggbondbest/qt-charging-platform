...
> 交付定位：本目录保留 PR67 的**离线历史选址模仿实验**，不接入生产推荐接口。
> 在线找桩统一使用 ChargePilot 的位置、预计到站时间、可用性、等待及价格评分。
> 体验账号没有历史用户映射；“以前更可能选哪个站”也不等于“现在最适合去哪个站”，
> 因此不伪造历史偏好、不叠加两个互相冲突的推荐入口。下文成绩属于原实验，不作为在线产品质量承诺。
