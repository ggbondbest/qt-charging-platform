...
| h06(split_6h) | 17,875 | **13.385** | **19.115** | 26.597 / 36.321 | 18.164 / 27.592(1.0)* | **+49.7%** | +26.3% |
| h24(split_24h) | 17,425 | **13.67** | **19.562** | 17.518 / 26.763 | 18.085 / 27.545(1.0)* | **+22.0%** | +24.4% |

\* 评审 P2#2 修正(§13):原 last-week 列在 h06/h24 用了错锚(回看固定 168h,未随目标小时平移),基线 MAE 被虚高,提升% 相应虚报;表内为按 `reference−168h+(h−1)h` 正确对齐后的基线,模型数字与冻结文件逐字未动。
...
| | last-week* | 18.164 | 0.6897 | 48.75 | 61.108 |
| h24(split_24h,17425) | 模型 gbdt | **13.67** | **0.5227** | **34.218** | **42.505** |
| | persistence | 17.518 | 0.6698 | 47.28 | 59.859 |
| | last-week* | 18.085 | 0.6915 | 48.713 | 61.113 |

\* last-week 行为评审 P2#2 修正后重算(同一 TEST 掩码、对齐锚点,`analysis/lastweek_baseline_fix.json`);h01 本就不受错位影响。

sMAPE 口径说明(如实报告):模型 sMAPE h01/h06/h24 = 0.9999/0.9982/1.0046,**劣于**基线
(0.6725/0.7789、1.1499/0.7808、0.7566/0.7800)。原因是近零负荷小时占样本相当比例,`2|e|/(|y|+|ŷ|)`
...
| h06(split_6h) | 17,875 | **12.975** | 19.469 | 0.4927 | 34.16 / 42.80 | 1.0 | **+51.2%** | +28.6% |
| h24(split_24h) | 17,425 | **13.181** | 19.848 | 0.5040 | 34.83 / 43.59 | 1.0 | **+24.8%** | +27.1% |
...
## 12. 第三轮:交付加固(区间校准工件 / 168h 探路 / 集成实验)

负责人裁决:空闲桩线让位给另一位 ML 成员,本线专注负荷交付的深化。三件事 + 一场集成实验,全部不消费 TEST 额度(除非 soup 转正获批)。

### 12.1 Split conformal 区间工件(已交付,离线层)

`conformal.py` → `outputs/ml_load/conformal_intervals.json`。对上线的 hgb-q50 点模型,在 VALIDATION 上逐时距算符号残差 y−pred,取 q10/q90 作非对称偏移,名义 80% 覆盖。
- 24 个时距经验覆盖全部 0.7999(校准集=评估集,自洽值),平均宽度 41.3 kW(h01 最窄 38.1,长时距 ≈41.5)。
- **硬约束**:契约 `validate_prediction` 把 point 键集精确钉死为 `{timestamp,value}`,结果键集同理——区间**不可能**进 `/predict/load` 响应,除非组长改契约。因此该工件定位为大屏/分析层(中位数与在线点输出逐比特一致,不会出现"线上线下两个数")。
- 诚实条款:conformal 的覆盖保证依赖"与校准分布同分布"假设,本场景是时间外推,TEST 时段若有漂移,实际覆盖会低于 80%;该风险已在 json 的 note 里标注。

### 12.2 168h 长窗特征探路:结论=否(不找组长动合同)

ml/README 允许"168 小时基线需从小时表另建"。`lag168_probe.py` 把"上周同一时刻功率"作为第 40 列加进契约矩阵,三个契约时距各训一个其余参数完全同 v0.4 的探针:

| 时距 | 39 列 shipped | 40 列 +lag168 | ΔMAE |
|---|---|---|---|
| h01 | 12.2033 | 12.1952 | −0.0081 |
| h06 | 13.2711 | 13.2901 | +0.0190 |
| h24 | 13.3524 | 13.4134 | +0.0610 |

增益在噪声量级且两负一正——24 列滞后 + 滚动统计已吸收周节律。**不值得为它申请 FEATURE_VERSION 放号**,L3 精简特征仍是唯一待组长裁决的合同事项。

### 12.3 Seed bagging:结构性无效(归档为负结果)

`train_bag.py`:复用 seed42 上线模型 + 补训 seed 7/2024(共 48 个新模型,10.3 分钟,349MB 候选包)。结果 24/24 个时距 ΔMAE **精确 = +0.0000**。
原因不是运气:HistGradientBoosting 无行/列采样,`early_stopping=False` 下 `random_state` 只影响分箱并列打破——同数据同超参不同种子 ≈ 复制模型,均值当然回到同一预测。
**教训入档**:给确定性 GBDT 做种子集成是空操作;要多样性只能扰动超参或改数据。负结果如实保留(`bag/bag_metrics.json`),TEST 未动。

### 12.4 参数扰动模型汤:结论=否,维持 v0.4(已裁决,TEST 未动)

`train_soup.py`(25.5 分钟):M1=上线配方,M2=31 叶子/lr0.02/900 迭代,M3=127 叶子/lr0.05/强 L2,M4=lr0.015/1200 迭代,等权、先 clip 后平均。

| 时距 | shipped 单模型 | 4 成员汤 | ΔMAE |
|---|---|---|---|
| h01 | 12.2033 | 12.2008 | −0.0025 |
| h06 | 13.2711 | 13.2940 | +0.0229 |
| h24 | 13.3524 | 13.3790 | +0.0266 |

24/24 时距里只有 8 个改善,中位 Δ +0.008 kW,量级=噪声;未过"三契约时距全改善"门槛,归档不转正,TEST 额度一分没花。
败因清楚:h24 上各成员裸分 M1 13.352 < M4 13.362 < M3 13.377 < M2 13.565——**上线配方本就是单点最优**,把更弱的成员平均进来只会稀释,等权汤没有免费午餐。
结合 12.2(长窗无信号)与 12.3(种子集成对确定性 HistGB 无效),本模型类内的提升路径判到尽头:v0.4 即本代终点。剩余空间全在合同门外——L3 精简特征等组长放 FEATURE_VERSION,以及在线端点接线。

## 13. PR 评审修复记录(五个 P2,2026-09-14)

外部评审基于 `19ae998` 提出 5 个 P2,全部核实成立并已修复;回归测试入 `data_analysis/tests/test_ml_load_review_fixes.py`(16 例,随 CI discover 收集)。

1. **P2#1 时间戳格式**:新增 `parse_utc` 只认无小数秒格式,公共契约/API 允许的 `.000Z` 整点会在参数校验通过后被预测器拒绝;历史窗口按字符串相等比较,语义相同写法被误判断档。修复:`common.parse_utc` 委托 `contracts.serving.parse_utc`,`build_feature_row` 的排序与连续性检查改为按解析后的时间点比对。测试覆盖 `Z`/`.000Z` 等价、混合格式同一输出、断档仍拒。
2. **P2#2 周基线错位**(影响报告数字,不影响模型):`last_week_predictions` 对所有时距固定回看 168h,而第 h 小时目标锚在 `reference+(h-1)h`——h06/h24 分别拿早 5/23 小时的值当基线,基线 MAE 虚高、"vs last-week" 提升率虚报(h06 曾报 50.0%,实为 28.6%)。修复:函数挪入 `common` 并加 `horizon` 参数,train/evaluate 同步;VALIDATION 重训复算=17.848/17.834/17.829(与第一轮原表一致,证实是重构引入的回归);TEST 侧只重算基线、冻结模型评分文件未动(`analysis/lastweek_baseline_fix.json` 独立留痕),修正后 18.095/18.164/18.085,提升率 33.8%/28.6%/27.1%。persistence 基线定义无锚点问题,不受影响。
3. **P2#3 审计失败仍发布缓存**:`prepare_data` 原先先写 `joined_usable.pkl` 再按审计结果返回失败,坏数据可被下游直接训练且覆盖好缓存。修复:`publish()` 审计通过才 tmp+`os.replace` 原子发布 pkl;summary 无条件落盘并带 `auditPassed` 标志;`train`/`evaluate` 经 `common.require_prepared`(标志+批次双门禁)读缓存。回归测试含"审计失败不得产出/替换可用缓存"。
4. **P2#4 冻结评分可被覆盖**:evaluate 输出名虽含批次段,重复运行相同模型/批次仍会 `open(w)` 覆盖。修复:`common.write_new_json` 文件存在即 `FileExistsError`;`interval_test_confirm` 同规则。本次重跑 P2#2 修正时该防线已实际拦截过一次(想重写 298aa3ee 评分文件被拒,遂以独立 note 文件留痕)。
