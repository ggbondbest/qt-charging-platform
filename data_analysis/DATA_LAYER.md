...
按老师清洗流程补充的执行入口、质量审计、11 个分组维度/3 组双维对比和真实 HDFS 验证，见 [数据清洗与准备验收指南](docs/cleaning_acceptance.md)。`charging_sample_7d_v2` 和 `charging_full_180d_v2` 均已完成真实 HDFS 验证；环境、命令、路径、输出和对账结果分别见 [7 天 HDFS 验证记录](docs/hdfs_sample_7d_validation.json) 与 [180 天 HDFS 验证记录](docs/hdfs_full_180d_validation.json)。继续使用现有 FastAPI，不改变下述已交付的查询表和公共契约；每份验证记录只代表其明确写出的批次、代码基线和运行环境。
...
Vue 页面、模型训练/评价和真实预测仍由后续开发完成。老师 Linux/Hadoop 环境中的 7 天样例和 180 天完整批次均已完成 HDFS 验收，并保留各自独立的新路径、批次和对账记录。
...
| `docs/cleaning_acceptance_validation.json` | 是 | 2026-09-13 新清洗规则与全量/样本的本地验收记录；HDFS 结果见下方独立记录 |
| `docs/hdfs_sample_7d_validation.json` | 是 | 2026-09-14 老师 Linux/Hadoop 环境中 7 天样例的 HDFS pipeline、分析、export、文件校验与独立统计对账记录 |
| `docs/hdfs_full_180d_validation.json` | 是 | 2026-09-14 老师 Linux/Hadoop 环境中 180 天完整批次的 HDFS pipeline、分析、export、文件校验、下载交接与独立统计对账记录 |
