...
> 完整第二阶段现在使用 [统一交付入口](../delivery/README.md)：`python -m data_analysis.delivery.cli serve`。
> 本页保留子系统调试说明。统一网页还包含运营统计、小时空闲桩预测及管理分析；不要启动多个8000端口服务。
> 找桩内部现复用PR66小时平均负荷辅助均衡评分；原配对实验仍是未加入该辅助项的固定基线协议。
