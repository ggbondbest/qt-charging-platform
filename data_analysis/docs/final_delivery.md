...
仅当生成结果引用不合法或缺少必要引用时，最多追加一次生成纠错，要求删除无证据支撑的句子并使用允许ID。
发生纠错时最多三次供应商调用，仍共享原60秒总预算，单次最多45秒且受剩余预算限制；网络、认证、限流不自动重试。
纠错结果仍须通过同一引用校验：引用属于本次检索结果，分析答复引用聚合证据；知识解释可以只引用项目口径，未通过则报错。
