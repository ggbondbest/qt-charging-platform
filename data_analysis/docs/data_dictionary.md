...
| tariffs | 城市当地小时电价；tariff_id | city_id+hour 唯一；购电价、零售电价、服务费，均为分/kWh |
| campaigns | 一段优惠活动；campaign_id | city_id → cities；有效期、每次优惠、预算（分） |
| calendar | 城市一天；city_id+business_date | 星期及模拟展会事件，非官方节假日表 |
| weather_hourly | 城市一个小时；city_id+recorded_at | 模拟温度 °C、湿度 %、天气、降水 mm |
...
- 停止原因：TARGET_REACHED / USER_STOPPED / DURATION_LIMIT（5 小时上限或批次收尾）。
...
采用固定顺序优先分配可用桩，设备利用率因此受分配策略影响，不代表某品牌更好。
