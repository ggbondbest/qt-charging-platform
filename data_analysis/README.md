...
新增 [ChargePilot 智能找桩系统](chargepilot/README.md)：Vue 网页、到站空桩/等待/服务概率预测、
...
新增七表多维分析、补能行为/服务瓶颈联动和20类清洗挑战。AI运营参谋通过全站右下角人物展开聊天，
在线由模型规划检索当前发布聚合、已有模型报告与项目知识，再生成带引用的回答；保留无Key的本地统计问答。
首次将 `data_analysis/ml/advisor/.env.example` 复制为同目录 `.env.local`，填自己的AIPing Key后重启统一服务；
模板预设AIPing与 `DeepSeek-V4.1-Flash`，配置有效时默认在线。网页保留外发说明，在线模式主动发送即授权发送本次问题、有限历史和所选检索上下文，不再逐次勾选同意框；
模型解读需结合来源复核；配置状态不等于真实供应商连通性验证。详细步骤见 [AI参谋](ml/advisor/README.md)，不新增服务器或训练任务。
当前配置环境已实测AIPing规划、问候和MySQL聚合RAG回答。解析器处理安全的格式差异；规划和生成各最多两次尝试，
格式/字段/截断/引用问题在各阶段共享一次纠错机会，正常两次、总计最多四次供应商调用，共享60秒预算。
网络、认证、限流、内容过滤/工具调用及外层协议或超大响应不自动重试；前端可主动重发原问题并保留上下文和新草稿。
仍须通过严格引用校验，诊断日志不记录问答或密钥；成功用例与有限纠错均不能保证外部API持续可用或模型判断始终正确。
完整七表聚合可[追加安装至已有匹配的统计 MySQL 库](docs/mysql_setup.md#追加七表高级统计)，不覆盖原表。
全量留本地，参谋按问题构建最多48项成组分析证据，加模型报告后总计最多64项，并从16段口径/方法知识中BM25检索最多4段；不会全量外发。
