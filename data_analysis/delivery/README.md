...
Vue行程 ↔ 独立MySQL业务库：预约/排队/叫号/充电/停止/模拟支付/积分
```

| 页面 | 可展示内容 |
| --- | --- |
| 运营总览 | 城市/日期筛选；电量、收入、用户、利用率、资源状态、队列、维修、站型/地区对比、清洗质量；地图和实际图表 |
| 智能找站 | 五城市25站，点选起点、目标电量、ETA、到站可用概率、条件等待/P90、服务成功率、评分解释与奖励 |
| 我的行程 | 选择→到站→排队/预约→充电→自动/手动停止→模拟支付→积分；多会话同步 |
| 智能分析 | 1/6/24小时负荷与空闲桩预测、用户流失风险、会话异常复核、实际TEST指标和模型限制 |
| 模拟控制台 | 管理员令牌验证、回放时钟、资源背景占用、积分规则；不修改只读统计批次 |
...
5. 智能找站：创建昵称，点起点、输入目标电量，展示推荐理由及内部负荷辅助说明。
6. 选择站点→行程→控制台推进时钟→到站→充电→目标自动停止→模拟支付→积分；暂停时钟便于讲解。
7. 可选两浏览器/无痕窗口展示FIFO、叫号超时释放及同步。普通同浏览器标签共享体验账号。
