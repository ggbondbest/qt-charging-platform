...
“你好”等在线问候同样由模型生成。未配置Key时可用本地统计问答；有效配置存在时网页默认在线，仍须每次勾选外发同意。
...
生成时提供本次允许的引用ID；引用无效或缺失时最多追加一次生成纠错，纠错后仍须通过严格校验。
通常两次模型调用，纠错时最多三次，共享原60秒预算、单次最多45秒；网络、认证、限流不自动重试。
实际连通和引用匹配不保证模型全部语义判断正确，仍需逐项复核。
...
| `POST /api/v1/intelligence/advisor` | 同源统计/RAG问答，返回正文、范围、聚合、知识及引用；在线须 `mode=online` 与本次同意，省略mode仍为离线 |
