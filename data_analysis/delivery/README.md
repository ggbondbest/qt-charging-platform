...
| 智能分析 | 1/6/24小时负荷与空闲桩预测、用户流失风险、会话异常复核、实际TEST指标；同页子标签「AI参谋」解释本地证据 |
...
`ml/load`、`ml/availability`、`ml/insights.py` 负责各模型，`ml/advisor` 解释已有聚合和报告；
`frontend/` 是唯一网页源代码。
...
AI参谋默认离线可用，不需要外部模型Key、额外服务端口或另跑训练。它读取实际发布统计和已验证的本地模型报告，
返回来源与限制；这里的“实际证据”仍来自本项目模拟业务批次，不是现场实测。
可选在线说明由服务端专属 `ML_ADVISOR_*` 配置及请求中的用户同意共同控制，详见
[AI参谋配置与数据边界](../ml/advisor/README.md)。在线只发送白名单聚合指标；用户的原始问题、自由备注和逐条业务明细不发送。
外部模型仅选择关注指标，答案与数字仍按本地证据生成。
不要复用其他工具的Key或把密钥写入前端。基础验收使用默认离线模式即可。
...
- AI参谋把既有运营聚合和模型报告整理成有来源的说明；不预测新的故障概率，不重训模型，不自动改配置或安排维修。
...
| `GET /api/v1/intelligence/advisor` | 离线默认模式、支持的问题、在线可用状态与外发说明；不返回密钥 |
| `POST /api/v1/intelligence/advisor` | 同源AI参谋；默认离线读取已发布证据，返回可核对的说明和来源；在线调用须显式启用及同意 |
...
5. 打开智能分析中的AI参谋，在默认离线模式询问运营或模型问题，对照来源核对结论；缺证据应明确提示。
