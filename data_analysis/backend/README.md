...
本模块提供只读统计基础服务，使用只读账号读取发布完成的 MySQL 批次，不在请求中启动 Spark，也不扫描数百万行 raw CSV。
**完整交付请用 [delivery](../delivery/README.md) 统一入口**：它向本模块注入已校验的负荷/空闲桩模型，预测返回200，并挂载智能找桩及Vue。
单独启动下面的基础服务不加载模型，仍明确返回503；本页下文能力声明与503示例指的是这个独立基础模式。
该接口不复用 Qt TCP JSON 契约，第一阶段 Qt 的 SQLite 不变。
