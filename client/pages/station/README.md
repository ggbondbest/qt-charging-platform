...
- `StationDetailPage`：站点详情**路由占位页**（任务 #7 仅打通跳转，展示列表
  透传的基础字段 + 返回；桩位列表/价格明细/预约入口属任务 #12/#17）。
...
- 模拟通道（当前默认）：5 条演示站点 + 关键字过滤，带约 400ms 模拟延迟驱动
  加载状态；
- 真实通道：服务端 `GET_STATIONS` 就绪后，`MainWindow` 已注入
  `ClientConnection`，仅需 `service->setLiveMode(true)` 即切换，结果解析为
  同一 `StationList` 信号，**页面 UI 逻辑零改动**。
- 真实通道的失败路径已有测试覆盖（`station_query_service`：服务端未实现该
  命令时页面得到友好错误而非异常）。
