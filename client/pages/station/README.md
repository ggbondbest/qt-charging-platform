...
- `HomeShell`：用户端根页面 = **TopNavBar（顶部）+ QStackedWidget（内容）+
  ...
  4. **我的**（账户卡片：昵称/手机号/余额透传 + “📒 我的预约”入口 + “退出登录”）。
  另含索引 4 的详情路由页与索引 5 的“我的预约”记录路由页（均非 Tab）。
  顶部导航“‹ 返回”按路由回上一级（详情 → 找站、记录 → 我的），由
  `HomeShell` 统一维护显示/收起。
...
- `ReservationDialog`（任务 #17）：站点详情页的预约弹窗（`setModal + show`，
  阻塞交互但不阻塞事件循环）。表单 = 预约时长下拉（30/60/90/120 分钟，默认
  60）+ 站点名称 / 充电桩编号 / 预估费用（电价 × 时长即时联动）展示；提交经
  `ReservationService`：提交中按钮禁用显示“提交中…”（loading），成功绿色
  提示后短暂停留自动关闭并发 `reserved`（详情页据此刷新桩状态：被预约桩
  置为“已预约”、空位数重算），失败红色展示原因（桩被抢占 / 预约冲突 /
  参数非法 / 网络错误）且弹窗保持可重试，支持“关闭”手动退出。
- `ReservationListPage`（任务 #17）：“我的预约”记录页，经“我的”Tab 入口
  路由进入。每条记录卡片 = 站点名称 / 充电桩编号 / 预约时间 / 预约时长 /
  预约状态（预约中 / 已完成 / 已取消 / 已过期）+ 费用；仅“预约中”提供
  【取消预约】，其余状态按钮置灰（tooltip 说明），取消成功后自动刷新列表
  体现状态变化。状态全覆盖：加载中 / 空记录（友好空页面）/ 接口或网络错误
  （提示 + 重试）；取消失败以页内红条展示不打断列表浏览。
...
- 预约详情/提交/取消走 `ReservationService`（`client/services/reservation/`），
  同一双通道模式：服务端已实现 `RESERVE_CHARGER` / `CANCEL_RESERVATION`
  （需登录会话），注入连接并 `setLiveMode(true)` 即无缝切换，端到端真实
  提交/取消已有测试覆盖（`reservation_service`）；预约列表命令协议尚未
  定义，live 通道按未知命令走友好错误，就绪后 UI 零改动。预约成功后模拟
  通道经 `StationQueryService::setMockChargerReserved` 反映到详情刷新
  （真实通道以服务端桩状态为准）。
