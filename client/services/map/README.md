...
6. **在案现状（2026-09-08 活体探测）**：仓库两把演示 key 现网均回 `status 190 无效的key`
   （控制台未开通 WebServiceAPI 或已过期）。请求链与降级链已实测正确——config 兜底真发请求→
   190→Toast+模拟路线，UI 零卡死；换发有效 key（env 或改配置文件）后代码零改动自动激活真链路。
